...
class Drone():
    ...
    def moveDSF(self, detectedMap):
        ...
        if(detectedMap.surface[self.x-1][self.y]==0 and self.x>0):
            #top clear
            self._queue.append(UP)
        elif(detectedMap.surface[self.x][self.y-1]==0and self.y>0):
            #left clear
            self._queue.append(LEFT)
        elif(detectedMap.surface[self.x+1][self.y]==0 and self.x<19):
            #down clear
            self._queue.append(DOWN)
        elif (detectedMap.surface[self.x][self.y + 1] == 0 and self.y < 19):
            # right clear
            self._queue.append(RIGHT)



        if(self.x==0 or self.x==19 or self.y==0 or self.y==19):
            lastMove = self._lastMoves.pop(0)
            if(lastMove == UP):
                nextMove = DOWN
            elif(lastMove == LEFT):
                nextMove = RIGHT
            ...
            nextMove = self._queue.pop()


        if(nextMove==UP):
            #go up
            self.x = self.x - 1
            self._lastMoves.append(UP)
        elif(nextMove==LEFT):
            #go left
            self.y = self.y - 1
            self._lastMoves.append(LEFT)
        elif(nextMove==DOWN):
            #go down
            self.x = self.x + 1
            self._lastMoves.append(DOWN)
        elif(nextMove==RIGHT):
            #go RIGHT
            self.y = self.y + 1
            self._lastMoves.append(RIGHT)
