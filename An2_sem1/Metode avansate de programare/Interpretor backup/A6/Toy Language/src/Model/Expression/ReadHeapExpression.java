...
public class ReadHeapExpression implements IExpression,IValue{
    ...
    public IType typeCheck(IDictionary<String, IType> typeEnv) throws exceptionInvalidValue {
        IType type = this.varName.typeCheck(typeEnv);
        if(type.equals(Type.Reference)){
            ReferenceType referenceType = (ReferenceType) this.varName.getType();
            return referenceType.getInner();
        }
        else{
            throw new exceptionInvalidValue("Read heap argument is not of Reference type!");
        }
    }
}
