...
void UI::listAll()
{
	DynamicVector<TElem> printArray;
	printArray = service.getAll();
	for (int i = 0; i < printArray.lenght(); i++) {
		std::cout << this->getDogPrintable(printArray.get_element(i)) <<std::endl;
	}
}

void UI::listUserAdoptionList()
{
	DynamicVector<TElem> printArray = service.getAdoptionList();
	for (int i = 0; i < printArray.lenght(); i++) {
		std::cout << this->getDogPrintable(printArray.get_element(i)) << std::endl;
	}
}
...
{
	DynamicVector<TElem> printArray, tempArray = service.getAll();
	TElem tempElement;
	for (int i = 0; i < tempArray.lenght(); i++) {
		tempElement = tempArray.get_element(i);
		if (tempElement.getBreed() == breed && tempElement.getVaccinationsCount() < vaccinations)
			printArray.add(tempElement);
	}
...
void UI::processCommand(std::string command)
{
	...
	int modeIndex = 1, nameIndex = 1, breedIndex = 2, dateIndex = 3, vaccinationsIndex = 4, photographIndex = 5, saveNameIndex = 1, listVaccinationsBreedIndex = 1, listVaccinationsIndex = 2;
	std::string commandExit = "exit", commandList = "list", commandMode = "mode", commandAdd = "add", commandDelete = "delete", commandUpdate = "update", constantModeA = "A", constantModeB = "B", commandSave = "save", commandNext = "next", commandMyList = "mylist";
	...
	if (command == commandList) {
		...
		if (service.getRunMode() == constantModeA) this->listAll();
