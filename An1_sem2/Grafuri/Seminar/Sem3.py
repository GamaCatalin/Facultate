...
def getParents(graph,vertex):
	
	"""
		Returns the parents of the vertices in the graph starting from the given vertex
	"""
	visited = set()
	visited.add(vertex)
	parents = list(range(len(graph.parseX())))
	vertices = [vertex]
...
def printTree(parentsList,root,tier):
	'''
	Returns the tree with the root in the given vertex
	'''
	if tier == 1: 
		print(root)
	for i in range(len(parentsList)):
		if parentsList[i] == root and i != root:
			print("---"*tier+">" , i)
			printTree(parentsList, i, tier + 1)
...
#print (getParents(g,s))


for i in range(0,5):
	print()
	parents=getParents(g,i)
	print(parents)
	print("------------")
	printTree(parents,i,1)
	print()
	print()
